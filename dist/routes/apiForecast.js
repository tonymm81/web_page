...
const check_search_time = (what_time) => {
    ...
    if (permission[0]) {
        if (String(req.query.city_name).length > 0) {
            try {
                ...
                next(new errorHalndler_1.ServerError(400, `Not find data for this search word ${e}`));
            }
        }
        else {
            next(new errorHalndler_1.ServerError(400, "Not find data for this search word "));
        }
    }
    else {
